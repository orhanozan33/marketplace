...
const loadDatabase = () => {
  if (fs.existsSync(dbPath)) {
    ...
    // Eğer notifications field'ı yoksa ekle (migration)
...
export const dbQuery = {
  ...
  getListingMessageUsers: (listingId) => {
    // Bu ilanla ilgili mesaj gönderen tüm kullanıcıları bul (ilan sahibi hariç)
    if (!db.messages || !Array.isArray(db.messages)) {
      return [];
    }
    ...
    db.messages.forEach(msg => {
      if (msg.listingId === listingId) {
        // İlan sahibi hariç, mesaj gönderen veya alan tüm kullanıcıları ekle
        if (msg.senderId !== listing.userId) {
          messageUsers.add(msg.senderId);
        }
        if (msg.receiverId !== listing.userId && msg.receiverId) {
          messageUsers.add(msg.receiverId);
        }
      }
    });
    
    return Array.from(messageUsers);
  }
